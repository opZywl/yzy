...
export function DailyToolStack({ tools, className }: { tools: Tool[]; className?: string }) {
  ...
  return (
    <div className={cn("grid grid-cols-[86px,1fr] gap-6 text-white", className)}>
      <div className="flex flex-col items-center gap-3">
        {tools.map((tool) => (
          <div
            key={tool.name}
            className="flex h-14 w-14 items-center justify-center rounded-[22px] border border-white/10 bg-[#14141b] p-3 shadow-[0_26px_80px_-60px_rgba(0,0,0,0.8)]"
          >
      ...
      <div className="flex flex-col gap-6">
        <div className="space-y-2">
          <p className="text-[12px] uppercase tracking-[0.75em] text-white/30">DAILY</p>
          <h3 className="text-[54px] font-semibold uppercase leading-[0.85] tracking-[0.05em] text-white">
            Tool
            <br />
            STACK.
          </h3>
        ...
        <div className="relative h-[220px] overflow-hidden rounded-[32px] border border-white/10 bg-[#15151b]">
          {isLoading ? (
            <div className="flex h-full flex-col items-center justify-center gap-2 text-[11px] uppercase tracking-[0.4em] text-white/40">
              <span className="animate-pulse">SYNCING</span>
              <span>ASTRONOMY FEED</span>
            </div>
          ) : (
            <Image src={imageSrc} alt={apodTitle} fill sizes="(min-width: 1024px) 320px, 100vw" className="object-cover" onError={markAsBroken} />
          )}
          <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/45 to-transparent px-5 py-4">
            <p className="text-[11px] uppercase tracking-[0.5em] text-white/65">{creditLine}</p>
            <p className="mt-2 text-sm font-semibold tracking-[0.08em] text-white/85">{apodTitle}</p>
          </div>
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <div className="relative overflow-hidden rounded-[32px] border border-white/10 bg-[#15151b]">
            <Image src="/images/image_2.jpg" alt="Ghost Town" fill sizes="(min-width: 1024px) 180px, 50vw" className="object-cover" />
            <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/45 to-transparent px-5 py-4">
          ...
          <div className="relative overflow-hidden rounded-[32px] border border-white/10 bg-[#15151b]">
            <Image src="/images/naruto24.jpg" alt="Naruto" fill sizes="(min-width: 1024px) 180px, 50vw" className="object-cover object-top" />
